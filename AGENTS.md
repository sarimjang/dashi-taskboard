...
- **Push allowlist / remote**: push-relay hook installed at `.beads/hooks/post-commit` (verified: an empty commit on `agents/evidence` pushed successfully). Allowlist is `main|agents/evidence` — Spectra change branches are not yet added because no change has been applied yet and the real branch-naming pattern under `worktrees_dir: .spectra/worktrees` hasn't been observed; add it to the `case` statement once the first change branch exists. `origin` (push target) is `https://github.com/sarimjang/dashi-taskboard.git`; `upstream` (read-only) is `https://github.com/chuspeeism/dashi-taskboard.git`. Push log: `.git/push-relay.log` (git-common-dir root, not the worktree top level).
- **`core.hooksPath`**: redirected to `.beads/hooks` by beads (confirmed shared identically by the `.spectra/blackboard` worktree — the push-relay hook's own allowlist is what keeps that sharing safe, per evidence-branch-bootstrap.md §2's warning). `post-commit` was an empty slot — beads only manages `pre-commit`/`post-merge`/`pre-push`/`post-checkout`/`prepare-commit-msg`, confirmed via `bd hooks --help` — so no collision.
