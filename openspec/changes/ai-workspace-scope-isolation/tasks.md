...
- [ ] 3.2 [P] 追蹤確認 `resolveMappedAiWorkspace()`（server/ai-chat-catalog.mjs，目前零呼叫點的死程式碼）在完成 2.1 後，因為共用同一個 `resolvedWorkspace()` helper，回傳的 `addDirectories` 同樣恆為空陣列，不需要另外修改（對應設計文件「死程式碼 resolveMappedAiWorkspace 隨 helper 一併修正，清理留給既有任務」）。驗證方式：在 handoff 記錄中寫下這項追蹤確認的結論（函式呼叫鏈與程式碼引用），作為本任務的完成證據；不在本 change 內執行 `resolveMappedAiWorkspace()`／`loadMappedWorkspaces()` 本身的死程式碼清理。
